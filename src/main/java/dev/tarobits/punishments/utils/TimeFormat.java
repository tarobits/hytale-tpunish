...
public class TimeFormat {
    ...
    public enum TimeUnit {
        YEARS {
            void add(TimeFormat t, int n) {
                t.years += n;
            }

            void sub(TimeFormat t, int n) {
                t.years -= n;
            }
        },
        MONTHS {
        ...
        abstract void add(TimeFormat t, int n);

        abstract void sub(TimeFormat t, int n);
    }
    ...
    public Boolean add(TimeUnit unit, int num) {
        TimeFormat prevValue = new TimeFormat(this);

        unit.add(this, num);
        this.recalculate();
        if (this.isInvalid()) {
            LOGGER.atWarning().log("Generated TimeFormat is invalid. Resetting to previous value!");
    ...
    public Boolean sub(TimeUnit unit, int num) {
        TimeFormat prevValue = new TimeFormat(this);
    ...
    public void recalculate() {
        ...
        ZonedDateTime start = Instant.now().atZone(zone);
    ...
    public static TimeFormat fromInstantToInstant(Instant startInst, Instant endInst) {
        ZoneId zone = ZoneId.systemDefault();
    ...
    public static TimeFormat fromZDTToZDT(ZonedDateTime startZDT, ZonedDateTime endZDT) {
    ...
    public static TimeFormat fromDurationString(String durationString) {
        TimeFormat timeFormat = new TimeFormat();
        if (durationString.contains("perm")) return timeFormat;
        durationString = durationString.replaceAll(" ", "").replaceAll("min", "n");
        ...
        for (char ch : singular) {
            ...
            switch (ch) {
                ...
                default -> throw new IllegalArgumentException("tarobits.punishments.time.error.unkmod");
            }
        }
        return timeFormat;
    }
    ...
    public String toDurationString(Boolean useYears, Boolean useMonths, Boolean useDays, Boolean useHours, Boolean useMinutes, Boolean shortForm) {
        if (this.isZero()) {
            return "0d";
        }
        Map<String, Map<Boolean, String>> strings = Map.of(
                "y", Map.of(true, "y", false, " years"),
                "m", Map.of(true, "m", false, " months"),
                "d", Map.of(true, "d", false, " days"),
                "h", Map.of(true, "h", false, " hours"),
                "min", Map.of(true, "min", false, " minutes")
        );
        StringBuilder durString = new StringBuilder();
        if (useYears && this.years != 0) {
            durString.append(this.years);
            durString.append(strings.get("y").get(shortForm));
            durString.append(" ");
        }
        if (useMonths && this.months != 0) {
            durString.append(this.months);
            durString.append(strings.get("m").get(shortForm));
            durString.append(" ");
        }
        if (useDays && this.days != 0) {
            durString.append(this.days);
            durString.append(strings.get("d").get(shortForm));
            durString.append(" ");
        }
        if (useHours && this.hours != 0) {
            durString.append(this.hours);
            durString.append(strings.get("h").get(shortForm));
            durString.append(" ");
        }
        if (useMinutes && this.minutes != 0) {
            durString.append(this.minutes);
            durString.append(strings.get("min").get(shortForm));
            durString.append(" ");
        }
        return durString.toString().trim();
    }
