...
public class ArgUtils {
    private static final Map<CustomArgumentType, String> argumentDefinitionMap = Map.ofEntries(
            Map.entry(CustomArgumentType.SHADOW, "-s"),
            Map.entry(CustomArgumentType.BROADCAST, "-b")
    );
    public static Map<CustomArgumentType, String> decodeArguments(@Nonnull String input, @Nonnull Integer numberOfParametersToExclude, @Nonnull List<CustomArgumentType> possibleTypes) {
        ...
        for (int i = numberOfParametersToExclude; i < individual.length; i++) {
            if (individual[i] == null) continue;
            HytaleLogger.forEnclosingClass().atInfo().log(individual[i]);
    ...
    private static Map<CustomArgumentType, String> condenseDefaultIntoSingular(@Nonnull List<CustomArgument> argumentList) {
        ...
        argumentList.forEach((t) -> {
            if (t.type == CustomArgumentType.DEFAULT) {
                result.append(t.content).append(" ");
            } else {
                argumentMap.put(t.type,t.content);
            }
        });
        argumentMap.put(CustomArgumentType.DEFAULT, result.toString().trim());
        return argumentMap;
    }

    private static CustomArgument getArgument(@Nonnull String input, @Nonnull List<CustomArgumentType> possibleTypes) {
        CustomArgumentType determinedType = CustomArgumentType.DEFAULT;
        for (CustomArgumentType t : CustomArgumentType.values()) {
            if (t == CustomArgumentType.DEFAULT) continue;
            if (!possibleTypes.contains(t)) continue;
            if (!input.contains(argumentDefinitionMap.get(t))) continue;
            determinedType = t;
            break;
        }
