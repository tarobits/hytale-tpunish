...
public class PunishmentEntryBuilder {
    ...
    public static String getActorName(UUID actor) {
        if (actor.toString().equals("00000000-0000-0000-0000-000000000000")) {
            return "Console";
        }
    ...
    public static String buildStatusText(@Nullable Punishment punishment, String displayText) {
        if (punishment == null) return "";
        if (punishment.getType() != PunishmentType.BAN && punishment.getType() != PunishmentType.MUTE) return "";
        if (punishment.isActive() && !punishment.isPardoned()) {
            ...
            if (punishment.getSubtype() == PunishmentSubtype.PERMANENT) {
                text.append(Message.translation("tarobits.punishments.gui.header.indefinitely").getAnsiMessage());
            } else {
                text.append(Message.translation("tarobits.punishments.gui.header.until").getAnsiMessage()).append(" ").append(punishment.getUntil());
            }
