...
public class PunishmentsGui extends InteractiveCustomUIPage<PunishmentsGui.PunishGuiData> {
	...
	public PunishmentsGui(
			...
			@Nonnull ProfileServiceClient.PublicGameProfile target
	) {
		...
		switch (data.button) {
			...
			case "UnbanButton":
				if (!pp.hasBan(target.getUuid())) {
					GuiUtil.sendToErrorPage(
							player, playerRef, ref, store, target,
							"tarobits.punishments.ban.error.isnt"
					);
					return;
				}
				try {
					player.sendMessage(pp.getActive(target.getUuid(), PunishmentType.BAN)
			...
			case "UnmuteButton":
				if (!pp.hasMute(target.getUuid())) {
					GuiUtil.sendToErrorPage(
							player, playerRef, ref, store, target,
							"tarobits.punishments.mute.error.isnt"
					);
					return;
				}
				try {
					player.sendMessage(pp.getActive(target.getUuid(), PunishmentType.MUTE)
							                   .pardon(playerRef.getUuid()));
				} catch (InvalidActionException e) {
					GuiUtil.sendToErrorPage(player, playerRef, ref, store, target, e.getMessage());
					return;
	...
	private void buildLists(
		...
		List<Punishment> banList = pp.getEntries(target.getUuid(), PunishmentType.BAN)
				.reversed();
		List<Punishment> muteList = pp.getEntries(target.getUuid(), PunishmentType.MUTE)
				.reversed();
		List<Punishment> warnList = pp.getEntries(target.getUuid(), PunishmentType.WARN)
				.reversed();
		List<Punishment> kickList = pp.getEntries(target.getUuid(), PunishmentType.KICK)
				.reversed();

		historyStats.put(
				PunishmentType.BAN,
				new HistoryStat(banList.size(), banList.isEmpty() ? null : banList.getFirst())
		);
		historyStats.put(
				PunishmentType.MUTE,
				new HistoryStat(muteList.size(), muteList.isEmpty() ? null : muteList.getFirst())
		);
		historyStats.put(
				PunishmentType.WARN,
				new HistoryStat(warnList.size(), warnList.isEmpty() ? null : warnList.getFirst())
		);
		historyStats.put(
				PunishmentType.KICK,
				new HistoryStat(kickList.size(), kickList.isEmpty() ? null : kickList.getFirst())
		);
	...
	private void checkPermissions(
		...
		if (!Permissions.playerHasAny(
				uuid, List.of(
						Permissions.BAN_PERM, Permissions.BAN_TEMP, Permissions.MUTE_PERM, Permissions.MUTE_TEMP,
						Permissions.KICK, Permissions.WARN
				)
		)) {
	...
	private void setEvents(@Nonnull UIEventBuilder uiEventBuilder) {
		uiEventBuilder.addEventBinding(
				CustomUIEventBindingType.Activating, "#NewPunishButton",
				EventData.of(PunishGuiData.BUTTON_KEY, "NewPunishButton")
		);
		uiEventBuilder.addEventBinding(
				CustomUIEventBindingType.Activating, "#UnbanButton",
				EventData.of(PunishGuiData.BUTTON_KEY, "UnbanButton")
		);
		uiEventBuilder.addEventBinding(
				CustomUIEventBindingType.Activating, "#UnmuteButton",
				EventData.of(PunishGuiData.BUTTON_KEY, "UnmuteButton")
		);
	}
	...
	public static class PunishGuiData {
		...
		public static final BuilderCodec<PunishGuiData> CODEC = BuilderCodec.builder(
						PunishGuiData.class,
						PunishGuiData::new
				)
				.append(new KeyedCodec<>(BUTTON_KEY, Codec.STRING), (data, u) -> data.button = u, (data) -> data.button)
				.add()
