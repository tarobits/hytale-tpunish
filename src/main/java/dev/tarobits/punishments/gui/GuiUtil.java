...
public class GuiUtil {
    ...
    protected static void buildPunishmentHeader(
        ...
        if (reset) {
            ...
            uiCommandBuilder.set(headerSelector + " #TypeDropdown.Entries", Arrays.stream(PunishmentType.values()).map(type -> new DropdownEntryInfo(LocalizableString.fromString(type.toDisplayString()), type.toJson())).collect(Collectors.toList()));
            uiCommandBuilder.set(headerSelector + " #SubtypeDropdown.Entries", Arrays.stream(PunishmentSubtype.values()).filter(p -> !p.equals(PunishmentSubtype.NULL)).map(type -> new DropdownEntryInfo(LocalizableString.fromString(type.toDisplayString()), type.toJson())).collect(Collectors.toList()));
        ...
        uiCommandBuilder.set(headerSelector + " #DurationBox.Visible", showSub && typesSelected.subtype == PunishmentSubtype.TEMPORARY);
        ...
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, headerSelector + " #TypeDropdown", EventData.of(ListPunishmentsData.ACTION_KEY, "TypeDropdown").append(ListPunishmentsData.TYPE_DROP_KEY, headerSelector + " #TypeDropdown.Value").append(ListPunishmentsData.SUBTYPE_KEY, headerSelector + " #SubtypeDropdown.Value"));
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, headerSelector + " #SubtypeDropdown", EventData.of(ListPunishmentsData.ACTION_KEY, "TypeDropdown").append(ListPunishmentsData.TYPE_DROP_KEY, headerSelector + " #TypeDropdown.Value").append(ListPunishmentsData.SUBTYPE_KEY, headerSelector + " #SubtypeDropdown.Value"));
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, headerSelector + " #SubmitButton", EventData.of(ListPunishmentsData.ACTION_KEY, "Submit")
        ...
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, headerSelector + " #ResetButton", EventData.of(ListPunishmentsData.ACTION_KEY, "Reset"));
    ...
    protected static class TypesSelected {
        ...
        public TypesSelected(@Nonnull PunishmentType type, @Nonnull PunishmentSubtype subtype) {
            this.type = type;
    ...
    protected static void buildPlayerHeader(
        ...
        for (Map.Entry<PunishmentType, HistoryStat> entry : historyStats.entrySet()) {
            String typeString = entry.getKey().toDisplayString();
            String statusText = PunishmentEntryBuilder.buildStatusText(entry.getValue().latestPunishment, entry.getKey().toDisplayString());
            ...
            uiCommandBuilder.set("#" + typeString + "Stats" + (entry.getValue().numberOfPunishments == 0 ? "None" : "Some") + ".Visible", true);
            uiCommandBuilder.set("#" + typeString + "Stats" + (entry.getValue().numberOfPunishments != 0 ? "None" : "Some") + ".Visible", false);
    ...
    protected static void buildTabs(@Nonnull ListTypes type, @Nonnull Map<PunishmentType, Integer> tabButtonMap, @Nonnull PunishmentType selectedTab, @Nonnull List<TPunishConfig.PresetConfig> presetConfigs, @Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder uiCommandBuilder, @Nonnull UIEventBuilder uiEventBuilder, @Nonnull Store<EntityStore> store, @Nullable PlayerRef playerRef) {
        ...
        for (PunishmentType p : PunishmentType.values()) {
            ...
            if (tabButtonMap.containsKey(p)) continue;
            ...
            uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, selector, EventData.of(ListPunishmentsData.ACTION_KEY, "TabButton").append(ListPunishmentsData.TYPE_KEY, p));
            i++;
        }
    ...
    protected static void buildTab(@Nonnull ListTypes type, @Nonnull Map<PunishmentType, Integer> tabButtonMap, @Nonnull PunishmentType selectedTab, @Nonnull List<TPunishConfig.PresetConfig> presetConfigs, @Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder uiCommandBuilder, @Nonnull UIEventBuilder uiEventBuilder, @Nonnull Store<EntityStore> store, @Nullable PlayerRef playerRef) {
        ...
        for (TPunishConfig.PresetConfig p : presetConfigs) {
            ...
            if (p.getType() != selectedTab) continue;
            if (inCurrentRow == 0) {
                uiCommandBuilder.appendInline("#Tab", "Group { FlexWeight: 1; LayoutMode: Left; Padding: (Left: 5, Top: 5, Right: 5); }");
            }
            ...
            if (type == ListTypes.EDIT) {
                ...
                uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, itemSelector + " #EditButton", EventData.of(ListPunishmentsData.ACTION_KEY, "Edit").append(ListPunishmentsData.PUNISHMENT_KEY, String.valueOf(totalNumber - 1)));
                uiCommandBuilder.set(itemSelector + " #ExecuteButton.Visible", true);
                uiCommandBuilder.set(itemSelector + " #ExecuteButton.Text", "Delete");
                uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, itemSelector + " #ExecuteButton", EventData.of(ListPunishmentsData.ACTION_KEY, "Delete").append(ListPunishmentsData.PUNISHMENT_KEY, String.valueOf(totalNumber - 1)));
            } else if (type == ListTypes.ADD) {
                uiCommandBuilder.set(itemSelector + " #ExecuteButton.Visible", true);
                uiEventBuilder.addEventBinding(CustomUIEventBindingType.Activating, itemSelector + " #ExecuteButton", EventData.of(ListPunishmentsData.ACTION_KEY, "Execute").append(ListPunishmentsData.PUNISHMENT_KEY, String.valueOf(totalNumber - 1)));
            }
            inCurrentRow++;
            if (inCurrentRow > 4) {
                inCurrentRow = 0;
    ...
    public static class ListPunishmentsData {
        ...
        public static final BuilderCodec<ListPunishmentsData> CODEC = BuilderCodec.builder(ListPunishmentsData.class, ListPunishmentsData::new)
                ...
                .append(new KeyedCodec<String>(REASON_KEY, Codec.STRING), (d,v) -> d.reason = v, (d) -> d.reason).add()
                .append(new KeyedCodec<String>(SUBTYPE_KEY, Codec.STRING), (d,v) -> {
                    try {
                        if (v == null) {
    ...
    public static void sendToErrorPage(@Nonnull Player player, @Nonnull PlayerRef playerRef, @Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nullable ProfileServiceClient.PublicGameProfile target, @Nonnull String errorMessageId) {
        player.getPageManager().openCustomPage(ref, store, new ErrorGui(playerRef, CustomPageLifetime.CanDismiss, target, errorMessageId, player));
    }
}
