...
public class AddPunishmentGui extends InteractiveCustomUIPage<GuiUtil.ListPunishmentsData> {
	...
	public void handleDataEvent(
			...
			@Nonnull GuiUtil.ListPunishmentsData data
	) {
		...
		switch (data.action) {
				...
				try {
					...
					if (hasPunishment != null) {
						GuiUtil.sendToErrorPage(
								...
								"tarobits.punishments." + hasPunishment + ".error.already"
						);
						return;
					}
					switch (pc.getType()) {
						case BAN -> pp.addEntry(
								created = Punishment.createBan(
										target.getUuid(), playerRef.getUuid(), pc.getReason(),
										pc.getDuration()
								));
						case MUTE -> pp.addEntry(
								created = Punishment.createMute(
										target.getUuid(), playerRef.getUuid(), pc.getReason(),
										pc.getDuration()
								));
						case KICK -> pp.addEntry(
								created = Punishment.createKick(target.getUuid(), playerRef.getUuid(), pc.getReason()));
						case WARN -> pp.addEntry(
								created = Punishment.createWarn(target.getUuid(), playerRef.getUuid(), pc.getReason()));
					}
					if (created == null) {
						throw new IllegalArgumentException("Something went wrong while creating punishment!");
					}
					PlayerRef targetPlayerRef = Universe.get()
							.getPlayer(target.getUuid());
					if (targetPlayerRef == null || targetPlayerRef.getReference() == null) {
						...
						if (pc.getType() == PunishmentType.BAN || pc.getType() == PunishmentType.KICK) {
							targetPlayerRef.getPacketHandler()
					...
					player.sendMessage(created.getSuccessMessage(Message.raw(target.getUsername())
							                                             .bold(true)));
				} catch (NoPermissionException | InvalidActionException e) {
			...
			case "Edit":
				GuiUtil.sendToErrorPage(
						player, playerRef, ref, store, target,
						"tarobits.punishments.system.error.wrongpage"
				);
				return;
			default:
				player.sendMessage(Message.raw("Something went wrong!"));
		...
		buildTab(
				GuiUtil.ListTypes.ADD, this.tabButtonMap, this.selectedTab, this.presetConfigs, ref, uiCommandBuilder,
				uiEventBuilder, store, playerRef
		);
	...
	protected void sendUpdate(
			@NullableDecl UICommandBuilder commandBuilder,
			@NullableDecl UIEventBuilder eventBuilder,
			boolean clear
	) {
