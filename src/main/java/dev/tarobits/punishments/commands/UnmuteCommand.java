...
public class UnmuteCommand extends CommandBase {
	...
	protected void executeSync(@Nonnull CommandContext ctx) {
		...
		if (!punishmentProvider.hasMute(uuid)) {
			ctx.sendMessage(Message.translation("tarobits.punishments.mute.error.isnt")
					                .param("name", userName));
			return;
		}
		try {
			ctx.sendMessage(punishmentProvider.getActive(uuid, PunishmentType.MUTE)
					                .pardon(ctx.sender()
							                        .getUuid())
					                .param("action", "tarobits.punishments.edit.actions.unmute")
